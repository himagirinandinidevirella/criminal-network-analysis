...
export async function findCriminalIdByName(name: string): Promise<string | null> {
  try {
    ...
    const person = res.items.find(
      (r) => r.kind === "Person" && String(r.name).toLowerCase() === name.toLowerCase()
    );
...
/** Trigger the simulated live alert (broadcast over WebSocket). */
...
export async function demoStatus(): Promise<{ data_seeded: boolean; persons: number; ready: boolean }> {
