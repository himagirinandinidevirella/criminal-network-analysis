...
import { User, Building2, Car, Landmark, MapPin, ChevronRight } from "lucide-react";
  ...
  if (results.length === 0) {
    return (
      <div className="glass rounded-2xl p-8 text-center text-sm text-text-muted">
        Type a query to search the criminal network.
      </div>
    );
  }
    ...
    <div className="space-y-2">
      {results.map((r, i) => {
        ...
        return (
          ...
          >
            <div className={`flex h-10 w-10 items-center justify-center rounded-xl bg-bg-tertiary ${KIND_COLOR[kind] ?? ""}`}>
              <Icon className="h-5 w-5" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold">{String(r.name ?? "—")}</p>
              <p className="text-xs text-text-muted">
                {kind}
                {r.crime_types ? ` · ${String(r.crime_types).slice(0, 40)}` : ""}
              </p>
            </div>
            {kind === "Person" && <RiskBadge level={levelFor(riskScore)} score={riskScore} />}
            {kind !== "Person" && r.hotspot_score !== undefined && (
              <span className="text-xs text-text-muted">hotspot {(Number(r.hotspot_score) * 100).toFixed(0)}</span>
            )}
