...
const CRIME_TYPES = ["Drug Trafficking", "Money Laundering", "Cyber Crime", "Extortion", "Robbery"];
const RISK_LEVELS = ["CRITICAL", "HIGH", "MEDIUM", "LOW"];
const LAYOUTS = ["cose", "dagre", "circle", "grid", "concentric"];

export default function CriminalNetworkMap({ graph, onNodeSelect, onFilterChange }: Props) {
  ...
  const runLayout = (name: string) => {
    setLayout(name);
    cyRef.current?.layout({ name } as cytoscape.LayoutOptions).run();
  };
  ...
  return (
    <div className="glass flex flex-col rounded-2xl">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border p-3">
        ...
        <GraphControls onZoomIn={() => zoom(1.2)} onZoomOut={() => zoom(0.8)} onFit={fit} onReset={reset} onScreenshot={screenshot} />
      </div>
      ...
      <div className="relative h-[560px] overflow-hidden rounded-b-2xl bg-bg-primary/60">
        ...
        {!graph && !loading && (
          <div className="absolute inset-0 flex items-center justify-center p-6">
            <ErrorState message="Unable to load the network graph" onRetry={() => onFilterChange?.({})} />
          </div>
        )}
        <div ref={containerRef} className="cy-container" aria-label="Criminal network graph" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-border px-4 py-2 text-[11px] text-text-muted">
        ...
        <span className="flex items-center gap-1"><span className="h-2 w-2 rotate-45 bg-[#22303E]" /> Organization</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-full bg-[#17645B]" /> Location</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-[#8A6D3B]" /> Vehicle</span>
        <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-[#5B6B7A]" /> Account</span>
        <span className="ml-auto">Node size = centrality · Click = details · Double-click = expand</span>
      </div>
    </div>
  );
}
